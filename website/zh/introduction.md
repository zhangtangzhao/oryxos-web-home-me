---
title: OryxOS 介绍
description: OryxOS 是开源的 Agent Harness OS —— 一个目录定义一个 Agent，一个底座运行一群 Agent，私有部署，数据不出域。
---
...
OryxOS 是开源的 **Agent Harness OS**（Agent 运行骨架操作系统），基于 Java 21 实现、面向企业场景私有部署。它装在企业自己的 K8s 或服务器上，作为统一底座运行各种业务 Agent —— 运维助手、客服助手、HR 助手、知识管理助手 —— 共享一套渠道接入、模型路由、工具调用、记忆系统、沙箱执行与审计。数据完全留在企业自己的基础设施，不锁任何云生态。

> **你用一句自然语言发布一个任务 → 底座把它拆解 → 组织一支 Agent 团队 → 多个 Agent 分工协作 → 交付一个结果。**

让一群 Agent 像一群进程跑在操作系统上一样，可靠地运行和协同。让每一家公司，都能用自然语言跑起来自己的 Agent。

## Agent Harness OS：模型外面那层骨架
...
裸模型只会生成文本，harness 才让它可靠、安全地「做事」。
...
一个目录定义一个 Agent，一个底座运行一群 Agent，私有部署，数据不出域。原生对接 MCP 与 A2A 开放协议。
...
|---|---|---|
| 管理对象 | 单个 Agent | 一群 Agent |
| 职责 | 调用模型、执行工具、管理上下文、控制推理循环 | 生命周期、统一渠道与接入、统一记忆、多租户与治理、跨节点协作 |
| 操作系统类比 | 单个进程的执行环境 | 管理一群进程、调度资源、提供共享服务的那一层 |

一句话：runtime 让一个 Agent 跑起来，Agent Harness OS 让一群 Agent 被运行和管理起来。OryxOS 是后者。

## 五大核心能力

| 能力 | 说明 |
|------|------|
| 🤖 **对接 LLM** | Provider 抽象统一对接主流大模型，Agent 不感知具体厂商，运行时切换无锁定，支持本地推理。多 Provider 并存通过显式映射区分 |
| 🧠 **ReAct 循环** | Agent 的推理引擎，自己实现不套外部框架。LLM 思考是否调工具、调哪个，OryxOS 执行后回填结果，直到给出最终响应或达到最大迭代次数，循环行为完全可控 |
| 💾 **记忆** | 让 Agent 跨对话保留状态。会话记忆加长期记忆两层，长期记忆用文件形态、关键词检索，接口预留向量检索升级空间 |
| 🔧 **工具体系** | Agent 通过工具操作外部世界。内置文件、Shell、HTTP 等基础工具；扩展三档接入，门槛从低到高：零代码（Agent 目录 + 复用 MCP server）、轻代码（自写 MCP server）、重代码（原生方法） |
| 🌐 **对外服务** | 所有能力通过 REST API 对外暴露，业务系统 HTTP 接入，任何开发语言都可集成 |
...
- ☕ **Java 原生** —— 基于Java 与 JDK 21，单可执行 JAR 单二进制部署，复用现有 Java 运维工具链
...
成为 Agent 时代的运行底座。让企业内的各类业务 Agent、跨团队协作的 Agent、跨节点分布的 Agent，都能运行、被管理、被协同在同一套底座之上 —— 并最终能听从一句自然语言的调度，以团队之力交付复杂任务。
