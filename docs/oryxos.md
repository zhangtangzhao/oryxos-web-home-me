...
OryxOS 是开源的 **Agent Harness OS**。agent harness（运行骨架）是套在模型外面、把模型变成能干活的 Agent 的那层脚手架：

- 驱动 Reason → Act → Observe 的循环
- 它能调用的工具与执行工具的机制
- 每次调用前组装好的上下文
- 它积累的记忆
- 约束它的沙箱
...
一个目录定义一个 Agent，一个底座运行一群 Agent，私有部署，数据不出域。原生对接 MCP 与 A2A 开放协议，覆盖模型接入、推理循环、记忆、工具调用、对外服务五大核心能力。让一群 Agent 像一群进程跑在操作系统上一样，可靠地运行和协同。
...
| 核心职责 | 调用模型、执行工具、管理上下文、控制推理循环 | 生命周期、统一渠道与接入、统一记忆、多租户与治理、跨节点协作 |
...
Agent 通过工具操作外部世界。内置文件、Shell、HTTP 等基础工具。扩展三档接入，门槛从低到高：

1. **零代码** —— 写 Agent 目录，复用现成 MCP server
2. **轻代码** —— 自写 MCP server
3. **重代码** —— 用原生方法

### 知识库

给 Agent 挂上私有文档检索。Markdown/TXT 文档按标题切分、嵌入向量化 + FTS 双路检索（加权 RRF 合并），返回带来源引用的片段。`AGENT.md` 写 `knowledge_bases` 即完成绑定，检索范围在服务端强制过滤——多 Agent 共用一个底座，各查各的库。嵌入模型变更会被身份防护拦截，拒绝静默重嵌。

除 CLI 与 REST API 外，浏览器打开 `/admin/` 即进入 **Web 管理台**（服务端托管静态页，无需单独部署）：知识库总览与单库详情、建库 → 添加文档 → 摄取就绪闭环、危险操作二次确认删除、单库试检索（页面上直接验证检索效果，与 Agent 工具同审计路径）。
